...
TKE=mean(abs(uhat_vec(1:fix(N/3),:)).^2)/N;

% t_trans=transient_time_detector(x(1:floor(length(x)*0.05)));
% t_trans=transient_time_detector(x(1:2000));
%t_trans=transient_time_detector(x);
% t_trans=transient_time_detector(x(1:floor(length(x)*0.05)));
...
hold on,
grid minor
plot([t_vec(t_trans),t_vec(t_trans)],[min(TKE),max(TKE)],'k-.','linewidth',1.5)
