...
M=  M+0.5*max([0,interpolate_val(x,inter_par{jj}) - K .* (R2-norm(x-xc)^2)+eps])^2;
    ...
    M=  M+0.5*max([0,interpolate_val(x,inter_par{jj}) - (K(1) + K(2)*norm(inter_par{jj}{2})).* (R2-norm(x-xc)^2)+eps])^2;
end
...
if length(K) ==1
    ...
    g=interpolate_grad(x,inter_par{jj})+2*K*(x-xc);
       ...
       cmi=max([0,interpolate_val(x,inter_par{jj}) - (K(1) + K(2)*norm(inter_par{jj}{2})) .* (R2-norm(x-xc)^2)+eps]);
    g=interpolate_grad(x,inter_par{jj})+2*(K(1) + K(2)*norm(inter_par{jj}{2}))*(x-xc);
end
M=  M+cmi*g;
end
...
cmi=max([0,interpolate_val(x,inter_par{jj}) - (K(1) + K(2)*norm(inter_par{jj}{2})) .* (R2-norm(x-xc)^2)+eps]);
g=interpolate_grad(x,inter_par{jj})+2*(K(1) + K(2)*norm(inter_par{jj}{2}))*(x-xc);
H=interpolate_hessian(x,inter_par{jj})+2*(K(1) + K(2)*norm(inter_par{jj}{2}))*eye(length(x));
end
