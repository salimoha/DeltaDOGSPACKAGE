...
global n Ain bin ms K Eps inter_par inter_Par xc R2 bnd1 bnd2
...
% [x,y,INFO,lambda] = snsolve( @SearchFungrad, x, Ain, bin, [], [], lb, ub, @ConstSearchFungrad, options);
[x,y,INFO,lambda] = snsolve( @SearchFungrad, x, [], [], [], [], bnd1, bnd2, @ConstSearchFungrad, options);
...
function [c,ceq,DC,DCeq]= ConstSearchFungrad(x)
   global inter_Par xc R2 ms
   for jj = 1:ms
   c(jj,1) = costSearch(x,inter_Par{jj},xc,R2); % Inequality constraints
   end
...
function [f,gradf] = SearchFungrad2(x)
% a=100;
% b=0.0001;
% f = a*x(2)+b*sum(x.^2);
