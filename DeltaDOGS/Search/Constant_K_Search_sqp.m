function [x y CS]=Constant_K_Search_sqp(x,inter_par, inter_Par,xc,R2,Search)
...
A = vertcat(Jl,Ain);
b = vertcat(-1.*CSl,bin - Ain*x);
%
options=optimoptions('quadprog','Display','none');
[p,y,exitflag,output,lambda]=quadprog(H,g,A,b,[],[],[],[],zeros(n,1),options);
if exitflag==-2
...
a=1;
 %Backtracking ???????????????????????????
 y=meritfunction(x,rho,inter_par,inter_Par,xc,R2);
 while 1
   x1=x+a*p;
         y1=meritfunction(x1,rho,inter_par,inter_Par,xc,R2);
         if (y-y1)>0
         x=x1; y=y1;
           break
         else
