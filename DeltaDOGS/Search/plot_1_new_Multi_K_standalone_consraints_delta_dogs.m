...
close all
clc
global n m ms bnd1 bnd2 Ain bin acon Search r xi tri K0 K1
r=2;
% parameters
n=2; ms=2; m=2*n; 
%iter_max=10;
...
%         G_c(ii,jj)=interpolate_val([xv(ii) ;xv(jj)],inter_par_g{1});
       ...
       CC(ii,jj) = max([G1, G2])- Search.constant * Err(ii,jj);
          ...
          [GMAX, idmax] =  max([G1, G2]);
             CC(ii,jj) = GMAX- (Search.constant(1) + Search.constant(2)*norm(inter_par_g{idmax}{2})) * Err(ii,jj);
       end
