...
gfun=@(x) (1*(x-c).^2+sin(w*((x-c)-0.1)))/50;
...
plot(xx,max(yy,gg)+.0003,'r-.','linewidth',2)
set(gca,'YTick',[])
set(gca,'XTick',[])
axis square 
...
plot(xx,p,'k--', 'linewidth',2)
 ...
 plot(xx,max(p,pg)+.0003, 'r-.', 'linewidth',2) 
 plot(xx,e*1+min(min(p,pg)),'r-')
 plot(xiu,max(pU,pgU),'k*', 'Markersize',10)
...
subplot(2,1,2)
% figure(4)
plot(xiu,sd*7,'k*', 'Markersize',10)
hold on
plot(xx(indc), sdm*7,'kp', 'Markersize',10, 'MarkerFacecolor', 'k')
 set(gca,'YTick',[])
set(gca,'XTick',[])
% axis square 
% grid on
