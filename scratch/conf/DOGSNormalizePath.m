function p = siNormalizePath(filepath)
%siNormalizePath dereferences drive ids and converts relative paths to be absolute paths
...
%
% For example:
% siNormalizePath('$KleisslLab4TB1') -> /mnt/lab_4tb1
% siNormalizePath('$$/local')        -> /path_to_code/conf/local
% siNormalizePath('local')           -> /path_to_code/conf/local  (after running def_addpath)
%
% default is to return the passed path

% accept cell input
if(iscell(filepath))
	p = cellfun(@siNormalizePath,filepath,'UniformOutput',0);
	return;
end

% default behavior:
global siforecast_reporoot;
p = filepath;
...
if(filepath(1)=='$')
	...
	if(filepath(2) == '$')
		...
		root = siforecast_reporoot;
		if(isempty(root))
			root = fileparts(which('def_addpath.m'));
		end
		p = [root filepath];
